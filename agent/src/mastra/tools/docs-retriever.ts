...
export const docsRetriever = createTool({
  id: "docsRetriever",
  description: "Search knowledge/<namespace> for relevant passages in .md|.mdx|.txt files.",
  ...
  execute: async (raw: any) => {
    // console.log("Executing docsRetriever with input:", raw);
    ...
    const namespace = (base?.namespace ?? "default").toString().trim();
